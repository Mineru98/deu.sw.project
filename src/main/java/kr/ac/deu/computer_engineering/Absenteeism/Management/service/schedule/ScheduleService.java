...
public class ScheduleService {
    ...
    // 일정 상세 조회
    @Transactional(readOnly = true)
    public Schedule getSchduleById(Long id) {
        Optional<Schedule> schedule = scheduleRepository.findById(id);
        if (schedule.isEmpty()) throw new CustomIllegalStateExceptionHandler("존재하지 않는 일정입니다.");
        return schedule.get();
    }
        ...
        if (user.isEmpty()) throw new CustomIllegalStateExceptionHandler("존재하지 않는 직원입니다.");
            ...
            if (dto.getTypeOfTask().equals("출근")) {
                ...
                if (!(dto.getApplyDateTime().isAfter(compareBeginDate) && dto.getApplyDateTime().isBefore(compareEndDate))) {
                    throw new CustomIllegalStateExceptionHandler("허용 되지 않는 범위의 출근 시간입니다.");
                }
                ...
                if (!(dto.getApplyDateTime().isAfter(compareBeginDate) && dto.getApplyDateTime().isBefore(compareEndDate))) {
                    throw new CustomIllegalStateExceptionHandler("허용 되지 않는 범위의 퇴근 시간입니다.");
                }
            }
            ...
            if (dto.getTypeOfTask().equals("휴가")) {
                ...
                if (dto.getTypeOfTask().equals("출근") && scheduleList.size() > 0) {
                    throw new CustomIllegalStateExceptionHandler("해당 일정에 이미 근무 기록이 있습니다.");
                } else if (dto.getTypeOfTask().equals("퇴근") && scheduleList.size() != 1) {
                    throw new CustomIllegalStateExceptionHandler("해당 일정에 출근 기록이 없습니다.");
                }
    ...
    // 일정 승락
    @Transactional
    public void approveSchedule(Long id, ScheduleDto dto) {
        Optional<Schedule> schedule = scheduleRepository.findById(id);
        if (schedule.isEmpty()) throw new CustomIllegalStateExceptionHandler("존재하지 않는 일정입니다.");
        schedule.ifPresent(t -> {
            t.setIsVerified(dto.getIsVerified());
